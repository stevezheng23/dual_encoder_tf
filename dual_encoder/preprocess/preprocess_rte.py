...
def preprocess(file_name,
    ...
    with open(file_name, "rb") as file:
        for line in file:
            ...
            if items[0] == "index" and items[1] == "sentence1" and items[2] == "sentence2" and items[3] == "label":
                continue
            
            source = spacy_tokenize(items[1].strip())
            target = spacy_tokenize(items[2].strip())
            label = "1" if items[3].strip() == "entailment" else "0"
